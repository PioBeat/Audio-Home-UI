...
//        echo $playlist;
	...
	public function loadPlaylist() {
		...
		exec( "mpc -h " . $this->server . " load " . basename( $this->filename, ".m3u" ));
	}

	public function listPlaylist() {
		exec( "mpc -h " . $this->server . " playlist " . basename( $this->filename, ".m3u" ), $output, $ret );
	...
	public function loadMP3() {
		exec( "mpc -h " . $this->server . " clear" );
		exec( "mpc -h " . $this->server . " listall | mpc -h " . $this->server . " add" );
		exec( "mpc -h " . $this->server . " update" );
	}
